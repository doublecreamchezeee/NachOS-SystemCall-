...
void ExceptionHandler(ExceptionType which)
{
	...
	case SyscallException:
		...
		case SC_Create:
		{
			...
			delete filename;
			increaseProgramCounter();			// người dùng thành công
			break;
		}
		...
		case SC_Send:
		{
			DEBUG(dbgSys, "Go send message socket!\n");
			...
			DEBUG(dbgSys, "Send mess success socket!\n");
			increaseProgramCounter();
			return;
			ASSERTNOTREACHED();
			break;
		...
		case SC_Receive:
		{
			DEBUG(dbgSys, "Go receive message socket!\n");
			//! For advanced part, we don't use this case
			int addrSocketid = kernel->machine->ReadRegister(4);
			int idbuffer = kernel->machine->ReadRegister(5);
			int len = kernel->machine->ReadRegister(6);
			...
			DEBUG(dbgSys, "Receive mess success socket!\n");
